...
## ---- 3.6 Formal DID Model ----

###--------------------------------------------------------------------------###
###   SECTION 3.6: True DiD — IPL vs Donor Leagues                           ###
###   (A-B) - (C-D) = (IPL post - IPL pre) - (Control post - Control pre)    ###
###--------------------------------------------------------------------------###

# Load donor league metrics built in synthetic control script
...
panel_did <-
  ...
  mutate(
    treated = if_else(league == "IPL", 1L, 0L),
    post    = if_else(season_yr >= 2023, 1L, 0L)
  )

# --- DiD Model 1: Run Rate ---
did_runrate <-
  lm(run_rate ~ post * treated, data = panel_did)

did_runrate_robust <-
  coeftest(did_runrate, vcov = vcovHC(did_runrate, type = "HC3"))

# --- DiD Model 2: All-rounders ---
did_allrounder <-
  lm(avg_allrounders ~ post * treated, data = panel_did)

did_allrounder_robust <-
  coeftest(did_allrounder, vcov = vcovHC(did_allrounder, type = "HC3"))

# --- DiD Model 3: Match Margin ---
did_margin <-
  lm(avg_margin ~ post * treated, data = panel_did)

did_margin_robust <-
  coeftest(did_margin, vcov = vcovHC(did_margin, type = "HC3"))

# Print results
cat("\n========== TRUE DiD RESULTS ==========\n")
cat("\nModel 1: Run Rate\n")
print(did_runrate_robust)

cat("\nModel 2: All-rounders per Match\n")
print(did_allrounder_robust)

cat("\nModel 3: Match Margin\n")
print(did_margin_robust)

cat("\nHow to read:\n")
cat("post           = time trend (did control leagues change post-2023?)\n")
cat("treated        = baseline IPL difference from control leagues\n")
cat("post:treated   = THE DiD ESTIMATE = (A-B)-(C-D)\n")
cat("                 This is the causal effect of the Impact Player rule\n")
cat("=======================================\n")

# --- DiD Visualisation ---
did_viz_data <-
  panel_did %>%
  group_by(season_yr, treated) %>%
  summarise(
    mean_run_rate    = mean(run_rate, na.rm = TRUE),
    mean_allrounders = mean(avg_allrounders, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(group = if_else(treated == 1, "IPL (Treated)", "Donor Leagues (Control)"))

fig_did_rr <-
  did_viz_data %>%
  ggplot(aes(x = season_yr, y = mean_run_rate,
             color = group, linetype = group)) +
  geom_line(linewidth = 1.3) +
  geom_point(size = 3) +
  geom_vline(xintercept = 2022.5, linetype = "dashed",
             color = "black", linewidth = 0.8) +
  annotate("text", x = 2023.1, y = 7.5,
           label = "Impact Player\nRule →",
  ...
  labs(
    title    = "DiD: Run Rate — IPL vs Donor Leagues (BBL, PSL, CPL)",
    subtitle = "Parallel trends pre-2023, divergence post-rule = causal effect",
...
fig_did_ar <-
  did_viz_data %>%
  ggplot(aes(x = season_yr, y = mean_allrounders,
             color = group, linetype = group)) +
  geom_line(linewidth = 1.3) +
  geom_point(size = 3) +
  geom_vline(xintercept = 2022.5, linetype = "dashed",
             color = "black", linewidth = 0.8) +
  annotate("text", x = 2023.1, y = 4.5,
           label = "Impact Player\nRule →",
           color = "#CE1141", size = 3, hjust = 0) +
  scale_color_manual(values = c("IPL (Treated)"            = "#CE1141",
                                "Donor Leagues (Control)"  = "#17408B")) +
  ...
  scale_x_continuous(breaks = seq(2015, 2025, by = 1)) +
  labs(
    title    = "DiD: All-rounder Usage — IPL vs Donor Leagues (BBL, PSL, CPL)",
    subtitle = "Parallel trends pre-2023, divergence post-rule = causal effect",
...
# ---- 5. Report ----
###--------------------------------------------------------------------------###
###   SECTION 3b: True DiD — IPL vs Donor Leagues                           ###
###   (A-B) - (C-D) = (IPL post - IPL pre) - (Control post - Control pre)  ###
###--------------------------------------------------------------------------###

# Load donor league metrics built in synthetic control script
# If not in environment, rebuild quickly here
if (!exists("all_leagues")) {
  bbl_metrics <- get_league_metrics("bbl", "BBL")
  psl_metrics <- get_league_metrics("psl", "PSL")
  cpl_metrics <- get_league_metrics("cpl", "CPL")
...
panel_did <-
  ...
  mutate(
    season_yr = case_when(
      str_detect(season, "/") ~ as.integer(str_extract(season, "^\\d{4}")),
      TRUE ~ as.integer(season)
    )
  ) %>%
  filter(season_yr >= 2015, season_yr <= 2025) %>%
  mutate(
